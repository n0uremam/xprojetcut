...
async function fetchPatterns() {
  try {
    ...
    apiAvailable = false;
    patterns = [];
    refreshDropdowns('', { type: '', brand: '', model: '', year: '', trim: '' });
...
function matchesSearch(p, search) {
  ...
  return (
    ...
    (p.tags || '').toLowerCase().includes(target) ||
    (p.brand || '').toLowerCase().includes(target) ||
    (p.model || '').toLowerCase().includes(target)
  );
}
...
function applyFilters() {
  ...
  const filtered = patterns.filter((p) => {
    ...
    return matchSearch && matchType && matchBrand && matchModel && matchYear && matchTrim;
  });

  refreshDropdowns(search, { type, brand, model, year, trim });
...
function refreshDropdowns(search, selections) {
  ...
  const typeCounts = countBy(searchFiltered, 'type');

  const brandBase = searchFiltered.filter((p) => !selections.type || p.type === selections.type);
  const brandCounts = countBy(brandBase, 'brand');

  const modelBase = brandBase.filter((p) => !selections.brand || p.brand === selections.brand);
  const modelCounts = countBy(modelBase, 'model');

  const yearBase = modelBase.filter((p) => !selections.model || p.model === selections.model);
  const yearCounts = countBy(yearBase, 'year');
  ...
  yearSelect.disabled = !selections.model || !Object.keys(yearCounts).length;
  trimSelect.disabled = !selections.year || !Object.keys(trimCounts).length;
}
