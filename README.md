...
The server logs `DB: cockroach` when connected to CockroachDB. If no database URL is provided, the server exits at startup rather than using a non-persistent fallback.

## Deploying to Fly.io
- Deploy with the included `fly.toml` and `Dockerfile`.
- Make sure `DATABASE_URL` (or `NETLIFY_DATABASE_URL`) is configured in Fly.io secrets.
