...
const backupDir = path.join(__dirname, 'patterns');
...
async function ensureBackupDir() {
  try {
    ...
    console.error('Unable to create backup directory', err);
  }
}

async function writeBackup(patterns) {
  try {
    await ensureBackupDir();
    const payload = { savedAt: new Date().toISOString(), patterns };
    await fs.writeFile(backupFile, JSON.stringify(payload, null, 2), 'utf8');
  } catch (err) {
    console.error('Unable to write backup', err);
  }
}

async function loadBackupIntoMemory() {
  ...
  try {
    ...
    if (parsed && Array.isArray(parsed.patterns)) {
      memoryStore.splice(0, memoryStore.length, ...parsed.patterns);
    }
  } catch (err) {
    // Ignore missing backups; log other errors
    if (err.code !== 'ENOENT') {
      console.error('Unable to read backup', err);
    }
  }
}
